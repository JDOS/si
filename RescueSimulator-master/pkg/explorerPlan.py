...
class ExplorerPlan:
    def __init__(self, maxRows, maxColumns, goal, initialState, name = "none", mesh = "square"):
        ...
        self.stateAntecessor = initialState
        self.untried = self.setUntried()
        self.result = self.initResult()
        #self.unbacktracked = self.setUnbacktracked()
    ...
    def setUntried(self):
        ...
        for i in range(self.maxRows):
            for j in range(self.maxColumns):
                self.untried[str(State(i,j))]=["N", "S", "L", "O", "NE", "NO", "SE", "SO"]

        for k,v in self.untried.items():
            print(k,v)

        print('teste')
        print(self.untried.get(str(State(0,0))))
    ...
    def initResult(self):
        ...
        for i in range(self.maxRows):
            for j in range(self.maxColumns):
                self.result[str(State(i,j))]={}
                for k in range(8):
                    self.result[str(State(i,j))][possibilities[k]] =None


        self.result[str(State(1,1))]["S"]= State(1,1)
    ...
    def setResult(self,stateAntecessor,action, state):
        self.result[str(stateAntecessor)][action]= state
 

    def removeUntried(self,action, state):
        list = self.untried[str(state)]
        list.remove(action)
        self.untried[str(state)]=list
    ...
    def updateCurrentState(self, state):
        self.stateAntecessor = self.currentState
